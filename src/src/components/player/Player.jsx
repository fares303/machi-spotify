  ...
  if (isFullscreen) return (
    <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <div className="flex-1 flex gap-8 px-8 pb-4 overflow-hidden">
          {/* Left: art + controls */}
          <div className="flex-1 flex flex-col items-center justify-center gap-6 min-w-0">
            <motion.div animate={{rotate:isPlaying?360:0}} transition={{duration:6,repeat:Infinity,ease:'linear'}}
              ...
              style={{width:260,height:260,boxShadow:'0 0 60px color-mix(in srgb, var(--color-primary) 25%, transparent)'}}>
              <img src={currentTrack.cover} alt="" className="w-full h-full object-cover"
                onError={e=>e.target.src='https://placehold.co/260x260/161622/0DFFB0?text=♫'}/>
            </motion.div>

            <Visualizer getAnalyserData={getAnalyserData} isPlaying={isPlaying} width={260} height={50}/>

            <div className="w-full max-w-sm text-center">
              <h2 className="font-display font-bold text-2xl truncate mb-1">{currentTrack.title}</h2>
              <p className="text-sm truncate" style={{color:'var(--color-on-surface-muted)'}}>{currentTrack.artist}</p>
            </div>

            {/* Progress */}
            <div className="w-full max-w-sm flex items-center gap-3">
              <span className="text-xs w-10 text-right" style={{color:'var(--color-on-surface-muted)'}}>{fmt(displayProg)}</span>
              ...
              <span className="text-xs w-10" style={{color:'var(--color-on-surface-muted)'}}>{fmt(duration)}</span>
            </div>
            ...
            <div className="flex items-center gap-5">
              <button onClick={()=>toggleFavorite(currentTrack)} style={{color:isFav?'#F87171':'var(--color-on-surface-muted)'}}><HiHeart className={`text-xl ${isFav?'fill-current':''}`}/></button>
              <button onClick={toggleMute} style={{color:'var(--color-on-surface-muted)'}} className="hover:text-white">{isMuted?<HiVolumeOff className="text-xl"/>:<HiVolumeUp className="text-xl"/>}</button>
              <input type="range" min={0} max={1} step={0.01} value={isMuted?0:volume} onChange={e=>setVolume(Number(e.target.value))} style={{width:90}}/>
              <button onClick={()=>downloadTrack(currentTrack.videoId,currentTrack.title)} style={{color:'var(--color-on-surface-muted)'}} className="hover:text-white"><HiDownload className="text-xl"/></button>
              <button onClick={toggleEqualizer} style={{color:showEqualizer?'var(--color-primary)':'var(--color-on-surface-muted)'}}><MdEqualizer className="text-xl"/></button>
            </div>
          </div>
